...
from jumpstarter.states import ActorStoppingState
...
invalid_states_for_restart = set([state for state in ActorState] +
...
async def test_cant_restart_from_invalid_state(state, model, state_machine,
                                               actor_state_machine):
...
async def test_can_restart_twice(state, subtests, state_machine,
                                 actor_state_machine, model):
    ...
    with subtests.test(
            f"can restart again after restarting once from {state} state"):
        await model.restart()
        assert model._restart_state == ActorRestartState.restarted
        model.m.assert_has_calls([
            call.restarting(ANY),
